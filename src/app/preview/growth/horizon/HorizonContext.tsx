...
export function HorizonProvider({ children }: { children: ReactNode }) {
  ...
  if (appliedCoupon === 'HORIZON20') {
    discountAmount = cartTotal * 0.2;
  } else if (appliedCoupon === 'AWWWARDS') {
  ...
  const applyCoupon = (code: string) => {
    ...
    if (upperCode === "HORIZON20" || upperCode === "AWWWARDS") {
      setAppliedCoupon(upperCode);
      showToast(`Promo code ${upperCode} applied.`);
    } else {
      setCouponError("Invalid promotional code.");
    }
  };

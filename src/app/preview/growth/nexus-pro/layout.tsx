...
import { ShoppingCart, X, Menu, Heart, User, Search, ArrowRight, Globe, Mail, MessageCircle } from "lucide-react";
import { useState, useEffect } from "react";
import { ShopProvider, useShop } from "./ShopContext";
import { motion, AnimatePresence } from "framer-motion";

function CartFlyout() {
  const { isCartOpen, setIsCartOpen, cartItems, removeFromCart, updateCartQuantity, cartTotal } = useShop();
          ...
          >
            ...
            {cartItems.length > 0 && (
              <div className="p-8 border-t border-white/10 bg-[#0a0a0a]">
                ...
                <Link
                  href="/preview/growth/nexus-pro/cart"
                  onClick={() => setIsCartOpen(false)}
...
function Navbar() {
  ...
  const navLinks = [
    { name: "Home", path: "/preview/growth/nexus-pro" },
    { name: "Collection", path: "/preview/growth/nexus-pro/products" },
    { name: "About", path: "/preview/growth/nexus-pro/about" },
    { name: "Contact", path: "/preview/growth/nexus-pro/contact" },
  ];
      ...
      >
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <Link href="/preview/growth/nexus-pro" className="text-2xl font-black tracking-tighter uppercase text-white hover:text-[#d4af37] transition-colors">
              NEXUS<span className="text-[#d4af37]">.</span>
            </Link>
          </div>
          ...
          <div className="flex items-center gap-6">
            <Link href="/preview/growth/nexus-pro/products" className="hidden sm:block text-white/70 hover:text-white transition-colors">
              <Search className="w-5 h-5" />
            </Link>
            <Link href="/preview/growth/nexus-pro/profile" className="hidden sm:block text-white/70 hover:text-white transition-colors">
              <User className="w-5 h-5" />
            </Link>
            <Link href="/preview/growth/nexus-pro/wishlist" className="text-white/70 hover:text-white transition-colors">
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="flex justify-between items-center mb-12">
              <Link href="/preview/growth/nexus-pro" onClick={() => setIsMobileMenuOpen(false)} className="text-2xl font-black tracking-tighter uppercase text-white">
                NEXUS<span className="text-[#d4af37]">.</span>
              </Link>
            ...
            <div className="flex flex-col gap-6 text-2xl font-black uppercase tracking-tighter">
              ...
              <Link href="/preview/growth/nexus-pro/profile" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-[#d4af37] transition-colors text-lg">My Account</Link>
              <Link href="/preview/growth/nexus-pro/products" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-[#d4af37] transition-colors text-lg">Search</Link>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 md:gap-8 mb-20">
          <div className="col-span-1 md:col-span-1">
            <Link href="/preview/growth/nexus-pro" className="text-3xl font-black tracking-tighter uppercase mb-6 block">
              NEXUS<span className="text-[#d4af37]">.</span>
            </Link>
            <p className="text-sm text-white/50 leading-relaxed mb-8 max-w-xs">
              Defining the future of premium aesthetics. Engineered for the modern individual who refuses to compromise on quality and design.
            </p>
          ...
          <div>
            <h4 className="text-xs uppercase tracking-widest font-bold mb-6 text-white/70">Shop</h4>
            <ul className="space-y-4 text-sm font-medium">
              <li><Link href="/preview/growth/nexus-pro/products" className="hover:text-[#d4af37] transition-colors">All Collection</Link></li>
              <li><Link href="/preview/growth/nexus-pro/products?category=Outerwear" className="hover:text-[#d4af37] transition-colors">Outerwear</Link></li>
              <li><Link href="/preview/growth/nexus-pro/products?wearType=accessory" className="hover:text-[#d4af37] transition-colors">Accessories</Link></li>
              <li><Link href="/preview/growth/nexus-pro/products?isNew=true" className="hover:text-[#d4af37] transition-colors">New Arrivals</Link></li>
            </ul>
          </div>
          
          <div>
            <h4 className="text-xs uppercase tracking-widest font-bold mb-6 text-white/70">Support</h4>
            <ul className="space-y-4 text-sm font-medium">
              <li><Link href="/preview/growth/nexus-pro/contact" className="hover:text-[#d4af37] transition-colors">Contact Us</Link></li>
              <li><Link href="/preview/growth/nexus-pro/privacy-policy" className="hover:text-[#d4af37] transition-colors">Privacy Policy</Link></li>
              <li><Link href="/preview/growth/nexus-pro/terms-conditions" className="hover:text-[#d4af37] transition-colors">Terms & Conditions</Link></li>
              <li><Link href="#" className="hover:text-[#d4af37] transition-colors">Shipping Returns</Link></li>
            </ul>
          </div>
          
          <div>
            <h4 className="text-xs uppercase tracking-widest font-bold mb-6 text-white/70">Newsletter</h4>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center pt-8 border-t border-white/10 text-xs text-white/40 font-medium">
          <p>&copy; {new Date().getFullYear()} Nexus Pro. All rights reserved.</p>
